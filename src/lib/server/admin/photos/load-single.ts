...
const PHOTO_SELECT =
  'id, gallery_id, galleries(slug), slug, title, capture_date, description, dimensions, license_text, seo_title, seo_description, og_title, og_description, og_image_path, status, deleted_at, updated_at';
const IMAGE_SELECT =
...
export const loadSinglePhotoEditorData = async (
  ...
  if (categoryLinksResult.error)
    throw error(500, categoryLinksResult.error.message);
  ...
  const categoryLinks = (categoryLinksResult.data ?? []) as Array<
    Pick<PhotoCategoryRow, 'category_id'>
  >;
  const tagLinks = (tagLinksResult.data ?? []) as Array<
    Pick<PhotoTagRow, 'tag_id'>
  >;
  const images = (imagesResult.data ?? []) as PhotoImageRow[];

  const hasReady = images.some((img: PhotoImageRow) =>
    Boolean(img.delivery_storage_path),
  );
  const hasPending = images.some(
    (img: PhotoImageRow) => !img.delivery_storage_path,
  );
  const photoConversionState: 'no-images' | 'pending' | 'ready' | 'mixed' =
    !images.length
      ? 'no-images'
      : hasReady && hasPending
        ? 'mixed'
  ...
  return {
    ...
    selectedCategoryIds: categoryLinks.map(
      (row: Pick<PhotoCategoryRow, 'category_id'>) => row.category_id,
    ),
    selectedTagIds: tagLinks.map(
      (row: Pick<PhotoTagRow, 'tag_id'>) => row.tag_id,
    ),
    images,
    photoConversionState,
    pendingConversionCount: images.filter(
      (img: PhotoImageRow) => !img.delivery_storage_path,
    ).length,
  };
};
