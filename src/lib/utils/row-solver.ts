...
function swapImprove(
  ...
  for (let pass = 0; pass < MAX_SWAP_PASSES; pass++) {
    ...
    for (let rA = 0; rA < rows.length && swaps < MAX_SWAPS_PER_PASS; rA++) {
      for (
        let rB = rA + 1;
        rB < rows.length && swaps < MAX_SWAPS_PER_PASS;
        rB++
      ) {
        for (
          let iA = 0;
          iA < rows[rA].length && swaps < MAX_SWAPS_PER_PASS;
          iA++
        ) {
          for (
            let iB = 0;
            iB < rows[rB].length && swaps < MAX_SWAPS_PER_PASS;
            iB++
          ) {
            // Swap
...
export function solveRows(
  ...
  if (photos.length === 0) {
    return {
      rows: Array.from({ length: R }, () => ({ photos: [], scale: 1 })),
      rowHeight,
    };
  }
...
export function solveColumns(
  photos: Array<{ id: string; aspect: number }>,
  numCols: number,
  viewportW: number,
  viewportH: number,
  gap: number,
