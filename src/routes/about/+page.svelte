<script lang="ts">
  ...
  const isSveditPage = $derived(data.page?.editor_mode === 'svedit');
  ...
  const setEditMode = (next: boolean) => {
    if (!canEditPublicly || !isSveditPage || !data.page) return;
  ...
  const onKeydown = (event: KeyboardEvent) => {
    ...
    if (!isShortcut || !canEditPublicly || !isSveditPage || !data.page) {
      return;
    }
...
<svelte:head>
  <title>{data.page?.title ?? 'About'}</title>
</svelte:head>
...
<CmsPageView page={data.page} editable={isEditMode} />
