...
<section class="mt-6 grid gap-8 lg:grid-cols-[360px_1fr]">
  <form method="POST" action="?/create" class="grid gap-3 rounded border border-black/10 p-4">
    ...
    <input name="sort_order" type="number" class="rounded border border-black/20 px-3 py-2" value="0" />
    ...
    {#each data.categories as category (category.id)}
      <form method="POST" action="?/update" class="grid gap-2 rounded border border-black/10 p-4">
        ...
        <div class="grid gap-2 sm:grid-cols-2">
          <input name="name" value={category.name} class="rounded border border-black/20 px-3 py-2" required />
          <input name="slug" value={category.slug} class="rounded border border-black/20 px-3 py-2" required />
        </div>
        <textarea name="description" class="rounded border border-black/20 px-3 py-2">{category.description ?? ''}</textarea>
        <div class="flex flex-wrap items-center gap-3">
          <input name="sort_order" type="number" value={category.sort_order} class="w-32 rounded border border-black/20 px-3 py-2" />
          <label class="flex items-center gap-2 text-sm">
