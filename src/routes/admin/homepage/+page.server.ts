...
import {
  homepageHeroSaveSchema,
  homepageSlideshowSaveSchema,
  homepageHeroRollbackSchema,
    ...
    throwLoaderError(
      {
        ...
        operation: 'auto-create home page record',
      },
      insertQuery.error,
    );
  }
...
export const load: PageServerLoad = async ({ locals, url }) => {
  ...
  if (pendingQuery.error) {
    throwLoaderError(
      {
        route: '/admin/homepage',
        operation: 'load pending conversion count',
      },
      pendingQuery.error,
    );
  }
  ...
  if (settingsQuery.error) {
    throwLoaderError(
      {
        route: '/admin/homepage',
        operation: 'load slideshow timing settings',
      },
      settingsQuery.error,
    );
  }
  ...
  if (homePage) {
    ...
    if (revisionsQuery.error) {
      throwLoaderError(
        {
          route: '/admin/homepage',
          operation: 'load home page revisions',
        },
  ...
  const [slideshowForm, rollbackForm] = await Promise.all([
    superValidate(zod4(homepageSlideshowSaveSchema)),
    superValidate(zod4(homepageHeroRollbackSchema)),
  ]);
...
export const actions: Actions = {
  save: async ({ locals, request }) => {
    const form = await superValidate(
      request,
      zod4(homepageSlideshowSaveSchema),
    );
    if (!form.valid) {
      return message(form, 'Validation failed.', { status: 400 });
    }

    const imageIds = parseUuidList(form.data.ordered_image_ids);
    ...
    const slideDurationMs = clampInt(
      Number(form.data.slide_duration_ms) || DEFAULT_SLIDE_DURATION_MS,
      SLIDE_DURATION_MIN_MS,
      SLIDE_DURATION_MAX_MS,
    );
    const transitionDurationMs = Math.min(
    ...
    const zoomStrengthPct = clampInt(
      Number(form.data.zoom_strength_pct) || DEFAULT_ZOOM_STRENGTH_PCT,
      ZOOM_STRENGTH_MIN_PCT,
      ZOOM_STRENGTH_MAX_PCT,
    );
    const panStrengthPct = clampInt(
      Number(form.data.pan_strength_pct) || DEFAULT_PAN_STRENGTH_PCT,
      PAN_STRENGTH_MIN_PCT,
      PAN_STRENGTH_MAX_PCT,
    );
  ...
  saveHero: async ({ locals, request }) => {
    ...
    const form = await superValidate(rawForm, zod4(homepageHeroSaveSchema));
    if (!form.valid) {
      const firstError =
        form.errors.title?.[0] ??
        form.errors.visibility_status?.[0] ??
        form.errors.editor_mode?.[0] ??
        'Validation failed.';
      return message(form, firstError, { status: 400 });
    }

    const heroVerticalAlignmentPct = clampHeroVerticalAlignmentPct(
      asOptionalNumber(rawForm.get('hero_vertical_alignment_pct')),
    );
    const result = await pagePayloadFromForm(rawForm);

    if (!result.ok) {
      applyFieldErrors(form, result.fieldErrors);
      form.data.hero_vertical_alignment_pct = String(heroVerticalAlignmentPct);
      return message(form, result.message, { status: 400 });
    }

    const visibilityStatus =
      asString(rawForm.get('visibility_status')).trim() === 'public'
        ? 'public'
        : 'draft';

    let homePageId = form.data.id.trim();
    if (!homePageId) {
  ...
  rollbackHero: async ({ locals, request }) => {
    const form = await superValidate(request, zod4(homepageHeroRollbackSchema));
    if (!form.valid) {
      const firstError =
        form.errors.id?.[0] ??
        form.errors.revision_id?.[0] ??
        'Validation failed.';
      return message(form, firstError, { status: 400 });
    }
