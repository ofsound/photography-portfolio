...
import { pageDeleteSchema, pageRollbackSchema } from '$lib/schemas/page';
...
const redirectToList = (msg: string) => {
	throw redirect(
		303,
		`/admin/pages?message=${encodeURIComponent(msg)}&success=1`,
	);
};

const redirectToEdit = (slug: string, msg: string) => {
	throw redirect(
		303,
		`/admin/pages/edit/${slug}?message=${encodeURIComponent(msg)}&success=1`,
	);
};
...
export const load: PageServerLoad = async ({ locals, params, url }) => {
	...
	const [deleteForm, rollbackForm] = await Promise.all([
		superValidate(zod4(pageDeleteSchema)),
		superValidate(zod4(pageRollbackSchema)),
	]);
...
export const actions: Actions = {
	update: async ({ locals, request }) => {
		...
		if (await isGallerySlugTaken(locals, result.payload.slug)) {
			return failForm('Slug conflicts with an existing gallery.', {
				fieldErrors: {
					slug: 'Slug conflicts with an existing gallery.',
				},
				values: result.values,
			});
		}
	...
	delete: async ({ locals, request }) => {
		const form = await superValidate(request, zod4(pageDeleteSchema));
		if (!form.valid) {
			return message(form, 'Missing page id.', { status: 400 });
		}
	...
	rollback: async ({ locals, request }) => {
		const form = await superValidate(request, zod4(pageRollbackSchema));
		if (!form.valid) {
			const firstError =
				form.errors.id?.[0] ??
