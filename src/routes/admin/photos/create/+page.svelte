<script lang="ts">
  ...
  const pendingConversionCount = $derived(data.pendingConversionCount as number);
...
<div class="flex flex-wrap items-center justify-between gap-3">
  <h1 class="text-xl uppercase tracking-[0.15em]">New Photo</h1>
...
<div class="mt-2 flex flex-wrap items-center gap-3 text-xs uppercase tracking-[0.12em]">
  <span class="rounded border border-border px-2 py-1">Pending conversions: {pendingConversionCount}</span>
  {#if pendingConversionCount > 0}
