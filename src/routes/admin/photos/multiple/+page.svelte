<script lang="ts">
  ...
  const uploadingCount = $derived(uploadQueue.filter((item) => item.status === 'uploading').length);
  const successCount = $derived(uploadQueue.filter((item) => item.status === 'success').length);
  const errorCount = $derived(uploadQueue.filter((item) => item.status === 'error').length);
  const queuedCount = $derived(uploadQueue.filter((item) => item.status === 'queued').length);
  ...
  const statusClasses = (status: UploadStatus) => {
    if (status === 'success') return 'border-success/40 bg-success-soft text-success';
    if (status === 'error') return 'border-danger/40 bg-danger-soft text-danger';
  ...
  const parseResponse = (payload: string): UploadResponse => {
    try {
      ...
      if (typeof parsed !== 'object' || parsed === null || typeof parsed.success !== 'boolean') return null;
      return {
  ...
  function uploadOne(item: UploadItem): Promise<void> {
    return new Promise((resolve) => {
      ...
      xhr.upload.addEventListener('progress', (event) => {
        ...
        item.progressPct = Math.max(1, Math.min(99, Math.round((event.loaded / event.total) * 100)));
      });
...
<p class="mt-2 text-sm text-text-muted">
  Upload multiple images at once. Each image creates one private draft photo with a lead image.
</p>

<section class="mt-6 grid gap-4 rounded border border-border p-4">
  <div class="grid gap-2">
    <p class="text-xs uppercase tracking-[var(--tracking-tight)]">Choose Images</p>
    <input
      type="file"
  ...
  <div class="flex flex-wrap items-center gap-2">
    <AdminButton variant="submit" type="button" disabled={!canStart} onclick={startUploads}>
  ...
  <div class="grid gap-2 rounded border border-border bg-surface-muted p-3 text-xs sm:grid-cols-5">
...
<section class="mt-6 grid gap-2">
  {#if uploadQueue.length === 0}
    <p class="rounded border border-border bg-surface p-3 text-sm text-text-muted">Select images to build the upload queue.</p>
  {:else}
    {#each uploadQueue as item (item.id)}
      <article class="grid gap-2 rounded border border-border p-3">
        <div class="flex flex-wrap items-center justify-between gap-2">
          ...
          <span class={`rounded border px-2 py-1 text-xs ${statusClasses(item.status)}`}>
            {statusLabel(item.status)}
          </span>
        </div>
        <div class="flex items-center gap-2">
          <div class="h-2 flex-1 overflow-hidden rounded bg-surface-muted">
            <div class={`h-full transition-all ${barClasses(item.status)}`} style={`width: ${item.progressPct}%;`}></div>
          </div>
          <span class="w-12 text-right text-xs tabular-nums text-text-muted">{item.progressPct}%</span>
        </div>
        {#if item.message}
          <p class={`text-xs ${item.status === 'error' ? 'text-danger' : 'text-text-muted'}`}>{item.message}</p>
        {/if}
      </article>
    {/each}
  {/if}
