...
<form method="POST" action="?/save" class="mt-6 grid max-w-[900px] gap-4 rounded border border-black/10 p-4">
  <div class="grid gap-3 sm:grid-cols-2">
    <label class="grid gap-1 text-xs uppercase tracking-[0.12em]">
      Theme Default
      <select name="theme_default" class="rounded border border-black/20 px-3 py-2">
    ...
    <label class="grid gap-1 text-xs uppercase tracking-[0.12em]">
      Layout Mode
      <select name="gallery_layout_mode" class="rounded border border-black/20 px-3 py-2">
  ...
  <div class="grid gap-3 sm:grid-cols-4">
    <label class="grid gap-1 text-xs uppercase tracking-[0.12em]">
      Desktop Default
      <input type="number" name="grid_desktop_default" value={settings.grid_desktop_default} class="rounded border border-black/20 px-3 py-2" />
    </label>
    <label class="grid gap-1 text-xs uppercase tracking-[0.12em]">
      Desktop Max
      <input type="number" name="grid_desktop_max" value={settings.grid_desktop_max} class="rounded border border-black/20 px-3 py-2" />
    </label>
    <label class="grid gap-1 text-xs uppercase tracking-[0.12em]">
      Mobile Default
      <input type="number" name="grid_mobile_default" value={settings.grid_mobile_default} class="rounded border border-black/20 px-3 py-2" />
    </label>
    <label class="grid gap-1 text-xs uppercase tracking-[0.12em]">
      Mobile Max
      <input type="number" name="grid_mobile_max" value={settings.grid_mobile_max} class="rounded border border-black/20 px-3 py-2" />
    </label>
  </div>
  ...
  <div class="grid gap-3 sm:grid-cols-3">
    <label class="grid gap-1 text-xs uppercase tracking-[0.12em]">
      Max Width (px)
      <input type="number" name="max_content_width_px" value={settings.max_content_width_px ?? ''} class="rounded border border-black/20 px-3 py-2" />
    </label>
    <label class="grid gap-1 text-xs uppercase tracking-[0.12em]">
      Uniform Thumb Ratio
      <input type="number" step="0.001" name="uniform_thumb_ratio" value={settings.uniform_thumb_ratio} class="rounded border border-black/20 px-3 py-2" />
    </label>
    <label class="grid gap-1 text-xs uppercase tracking-[0.12em]">
      Transition Preset {#if data.role !== 'admin'}(Admin){/if}
      <select name="transition_preset" class="rounded border border-black/20 px-3 py-2" disabled={data.role !== 'admin'}>
  ...
  <label class="grid gap-1 text-xs uppercase tracking-[0.12em]">
    ...
    <textarea
      ...
      rows="8"
      class="rounded border border-black/20 px-3 py-2 font-mono text-xs"
      disabled={data.role !== 'admin'}
    >{palettePretty}</textarea>
  </label>

  <button class="w-fit rounded border border-black/20 px-3 py-1 text-xs uppercase tracking-[0.14em]" type="submit">Save Settings</button>
</form>
