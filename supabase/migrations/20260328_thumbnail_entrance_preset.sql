...
alter table public.site_settings
  add column if not exists thumbnail_entrance_preset text not null default 'cascade';

alter table public.gallery_settings
  add column if not exists thumbnail_entrance_preset text not null default 'cascade';

update public.site_settings
set thumbnail_entrance_preset = coalesce(nullif(thumbnail_entrance_preset, ''), 'cascade')
